...
/**
 * 最終CTAの直前に出す一文（申し込みを後押しする案内）。
 * 文節ごとに分けているのは、スマートフォンで折り返したときに
 * 「ご相談下さい」のような語の途中で改行されないようにするため。
 */
...
export const HERO_IMAGE: {
  ...
  alt:
    "外構・外壁塗装・リフォーム会社専門。集客出来るホームページを、20万円で。" +
    "制作して終わりではなく、問い合わせ獲得まで設計。集客できなければ全額返金保証。" +
    "最短3日で集客。1ヶ月で0件から22件。" +
...
/* ═══════════════════════════════════════════════════════════════════════════
   ▼▼▼ 代表者（特徴セクションに表示） ▼▼▼
   name と photo の両方が入ったときだけブロックが出る。
   実在の人物を載せる箇所なので、名前も写真も確定するまでは表示しない
   （仮の氏名やフリー素材の顔写真は絶対に入れないこと）。

     photo   : public に置いた画像のパス（例: /images/adofy/representative.webp）
     photoW/H: その画像の実寸。丸くトリミングして表示するので正方形が望ましい
   ═══════════════════════════════════════════════════════════════════════════ */
export const REPRESENTATIVE: {
  ...
  photoW: 480,
  photoH: 480,
  // 一人称のコメントなので、本人の確認を取ってから公開すること
  message:
    "ホームページは作って終わりではありません。公開したあとに問い合わせが届くところまで、責任を持って設計します。",
};
...
/*
  料金は20万円の1プランのみ。
  プランを増やす場合はこの配列に足すだけで、料金表は自動で複数列に戻る
  （PricingSection が PLANS.length を見てレイアウトを切り替える）。
*/
export const PLANS: Plan[] = [
  {
    ...
    price: 20,
    featured: true,
    for: "ホームページから継続的に問い合わせを獲得したい建設業の事業者向け",
    items: [
      { text: "オリジナルデザイン" },
      { text: "5ページ程度" },
      { text: "スマートフォン対応" },
...
/* ═══════════════════════════════════════════════════════════════════════════
   ▼▼▼ SNS広告限定の割引 ▼▼▼
   現在は 0（＝割引なし）。バッジ・割引後価格・注記はページから消えている。
   再開する場合はここに割引率を入れるだけでよい（表示側の実装は残してある）。
   ═══════════════════════════════════════════════════════════════════════════ */
export const AD_DISCOUNT_PERCENT = 0;
