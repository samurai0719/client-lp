  ...
  return (
    <section className="relative px-4 sm:px-6 py-16 md:py-20">
      <div className="max-w-2xl mx-auto">
        <motion.div
          className="rounded-2xl bg-[#eaf3ee] border border-[#bfe0cd] p-6 sm:p-9 flex flex-col gap-4"
        ...
        >
          {POINTS.map((point) => (
            <div key={point.text} className="flex items-start gap-3">
              <point.icon className="w-5 h-5 text-[#1f4d3d] mt-0.5 shrink-0" aria-hidden="true" />
              <p className="text-[14px] sm:text-base font-semibold text-[#10302a] leading-relaxed">{point.text}</p>
            </div>
          ))}
        </motion.div>
      </div>
